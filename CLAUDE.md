...
Phase 7 adds archive and history features. Key requirements:

### Scope
- View past years' cards (read-only after year ends)
- Card statistics (completion rate, bingos achieved)
- View friends' historical cards (not just active card)

### New API Endpoints Needed
```
GET /api/cards/archive              - List all past (non-current year) cards
GET /api/cards/{id}/stats           - Get card statistics
GET /api/friends/{id}/card?year=X   - View friend's card for specific year
GET /api/friends/{id}/cards         - List friend's available card years
```

### Backend Changes
- `CardService`: Add `GetArchive()`, `GetStats()` methods
- `FriendHandler.GetFriendCard`: Add optional `year` query param, currently only returns most recent finalized card (see `internal/handlers/friend.go:314-321`)
- Consider adding `CardStats` model with: total_items, completed_items, completion_rate, bingo_count, first_completion_date, last_completion_date

### Frontend Changes
- Add archive page route (`#archive`)
- Add `App.renderArchive()` method
- Add year selector when viewing friend's card
- Visual distinction for archived vs current cards (e.g., sepia tone, "archived" badge)
- Stats display component

### Database
- No schema changes needed - `bingo_cards.year` and `bingo_cards.is_finalized` already support historical cards
- Stats can be computed from existing `bingo_items` data

### Current Limitations to Address
- `GET /api/friends/{id}/card` only returns the most recent finalized card
- No way to list which years a friend has cards for
- No statistics calculation anywhere in codebase
